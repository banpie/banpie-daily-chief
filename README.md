...
公开预览版可从 [GitHub Releases](https://github.com/banpie/banpie-daily-chief/releases) 下载通用 Skill、WorkBuddy 或 Codex 压缩包。包内安装器会从固定版本标签安装本地运行时，用户不需要操作终端。
...
- `0.2` SQLite 任务管理器、七步向导和网页：已实现，待更多新机验收。
- `0.3` WorkBuddy/Codex/通用 Skill 包：已实现离线包和等价夹具，待两个宿主真实定时公测。
- `1.0`：只有全部真实验收门槛通过后发布。
